...
@section('content')
    <div class="tabs-container">
        ...
        <div class="tabs-content">
            ...
            <x-data-table
                title="Project"
                description="Manage approved projects."
                createButtonText="Transfer Project"
                createButtonRoute="{{ route('pages.project.transfer.create') }}"
            ...
            >
                ...
                <tr style="{{ $project->status !== 'Active' ? 'background-color: #fff3cd;' : '' }}">
                    <td>{{ $project->project_number }}</td>
                    <td>{{ $project->project_year }}</td>
                    <td>{{ $project->name }}</td>
                    <td>{{ $project->parliament?->name ?? $project->dun?->name ?? '-' }}</td>
                    <td style="text-align: right;">{{ number_format($project->total_cost, 2) }}</td>
                    <td>{{ $project->approval_date ? $project->approval_date->format('d/m/Y') : '-' }}</td>
                    <td>
                        @if($project->status === 'Active')
                            <span class="status-badge status-active">Active</span>
                        @elseif($project->status === 'Projek Dibatalkan')
                            <span class="status-badge" style="background-color: #6c757d; color: white;">{{ $project->status }}</span>
                        @else
                            <span class="status-badge" style="background-color: #ffc107; color: #856404;">{{ $project->status }}</span>
                        @endif
                    </td>
                    <td>
        ...
        function viewProject(id) {
            ...
            fetch(`/pages/project/${id}`)
                ...
                .then(data => {
                    ...
                    // Handle Project Changes - use same format as NOC detail page
                    ...
                    if (data.nocs && data.nocs.length > 0) {
                        // Created NOC - use same design as Approval History with ribbon
                        let nocHtml = '';
                        data.nocs.forEach(noc => {
                            // Determine ribbon color based on status
                            let ribbonColor = '#007bff'; // Default blue
                            let statusColor = '#007bff';
                            if (noc.status === 'Approved') {
                                ribbonColor = '#28a745'; // Green for approved
                                statusColor = '#28a745';
                            } else if (noc.status === 'Rejected') {
                                ribbonColor = '#dc3545'; // Red for rejected
                                statusColor = '#dc3545';
                            } else if (noc.status === 'Draft') {
                                ribbonColor = '#6c757d'; // Gray for draft
                                statusColor = '#6c757d';
                            }
                            ...
                            // Ribbon section with status color
                            ...
                            nocHtml += '</div>';
                            nocHtml += '</div>';
                            nocHtml += '</div>'; // Close ribbon div
                            nocHtml += '</div>'; // Close main div
                        });
